...
export const GeofenceManager = memo(function GeofenceManager({
  ...
  useEffect(() => {
    if (pendingGeometry) setOpen(true);
  }, [pendingGeometry]);

  const { data: geofences = [], isLoading } = useGeofencesQuery(true);
  ...
  const createMutation = useMutation({
    ...
    onError: (err: Error) => {
      setSaveError(err instanceof ApiError ? err.message : "Failed to save geofence");
    },
  });
  ...
  const startDraw = (type: GeofenceType) => {
    setSaveError(null);
    onClearPending();
    onDrawModeChange(drawMode === type ? null : type);
    setOpen(true);
  };
  ...
  return (
    <>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={cn(
          "inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs backdrop-blur-sm",
          open || drawMode
            ? "border-cyan-500/40 bg-cyan-500/15 text-cyan-400"
            : "border-[var(--dash-border)] bg-[var(--surface-deep)]/90 text-[var(--dash-text)] hover:bg-[var(--dash-hover)]",
        )}
      >
      ...
      {drawMode && !open && (
        <div className="pointer-events-none absolute left-3 top-14 z-10 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-[10px] text-amber-300 backdrop-blur-sm">
          {drawMode === "CIRCLE" && "Click map to place circle center"}
          {drawMode === "RECTANGLE" && "Click two corners on the map"}
          {drawMode === "POLYGON" && "Click points on map, double-click to finish"}
        </div>
      )}
      ...
      {open && (
        <div className="absolute bottom-4 right-4 z-20 max-h-[70vh] w-[320px] overflow-y-auto rounded-xl border border-[var(--dash-border)] bg-[var(--surface-deep)]/98 p-4 shadow-xl backdrop-blur-sm">
          ...
          <p className="mt-2 text-[10px] text-[var(--dash-muted)]">
            1. Choose shape → 2. Draw on map → 3. Name & save
          </p>
          ...
          {pendingGeometry && (
            <div className="mt-3 space-y-2 rounded-lg border border-cyan-500/30 bg-cyan-500/5 p-3">
              <p className="text-[10px] font-medium uppercase text-cyan-400">
                New {pendingGeometry.type.toLowerCase()} ready
              </p>
              <input
                placeholder="Geofence name"
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full rounded-lg border border-[var(--input-border)] bg-[var(--input-bg)] px-2 py-1.5 text-xs text-[var(--dash-text)]"
              />
              ...
              {saveError && (
                <p className="rounded-lg border border-red-500/30 bg-red-500/10 px-2 py-1.5 text-[10px] text-red-400">
                  {saveError}
                </p>
              )}
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 rounded-lg border border-[var(--dash-border)] py-2 text-xs"
                >
                  Cancel
                </button>
                <button
                  ...
                  onClick={handleSave}
                  className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-cyan-600 py-2 text-xs text-white hover:bg-cyan-500 disabled:opacity-50"
                >
